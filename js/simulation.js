    ...
    function step() {
      ...
      if (currentMinute >= totalMinutes) {
        ...
        if (matchData.events.length > 0) {
          ...
          const userStr = userScorers.length > 0 ? `<span class="t-summary-label" data-i18n="${userScorers.length === 1 ? 'gol_scored' : 'gols_scored'}">${userScorers.length === 1 ? (TRANSLATIONS[state.lang].gol_scored || 'GOL') : (TRANSLATIONS[state.lang].gols_scored || 'GOLS')}</span> <span class="t-summary-names">${[...new Set(userScorers)].join(', ')}</span>` : '';
          const oppStr = oppScorers.length > 0 ? `<span class="t-summary-label" data-i18n="sofreu_goals">${TRANSLATIONS[state.lang].sofreu_goals || 'SOFREU'}</span> <span class="t-summary-names">${[...new Set(oppScorers)].join(', ')}</span>` : '';
